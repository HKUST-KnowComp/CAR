...
from data_utils import handle_underscores
...
def convert_examples_to_features(examples, tokenizer, max_length=512):
    ...
    for example in examples:
        ...
        data.append([input_ids, input_ids, example['correct']])
    return data
...
def mCollateFn(batch):
    ...
    for f in features:
        ...
        for i in range(len(f[0])):
            sequence = f[0][i] + [mask_token] * (max_len - len(f[0][i]))
            att_mask = [1] * len(f[0][i]) + [0] * (max_len - len(f[0][i]))
            label_sequence = f[1][i] + [mask_token] * (max_len - len(f[1][i]))
            batch_input_ids[-1].append(sequence)
            batch_input_mask[-1].append(att_mask)
            batch_input_labels[-1].append(label_sequence)

    batch_input_ids = torch.tensor(batch_input_ids, dtype=torch.long)
    batch_input_mask = torch.tensor(batch_input_mask, dtype=torch.long)
    batch_input_labels = torch.tensor(batch_input_labels, dtype=torch.long)
    batch_label_ids = torch.tensor(batch_label_ids, dtype=torch.long)
    return batch_input_ids, batch_input_mask, batch_input_labels, batch_label_ids
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    curr_best = 0.0
    CE = torch.nn.CrossEntropyLoss(reduction='none', ignore_index=tokenizer.mask_token_id)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss = loss_fct(ce_loss, batch[3].cuda())
...
def main():
    ...
    if args.do_train:
        ...
        os.system("cp run_pretrain_gpt2.py %s" % os.path.join(args.output_dir, 'run_pretrain_gpt2.py'))
